...
def delete_user(user_id):
    ...
    try:
        ...
        # If this user belongs to a Member account,
        # handle the Member record as well.
        member = user.member
        ...
        if member:
            ...
            if transaction:
                flash(
                    "This user belongs to a member with "
                    "transaction history and cannot be deleted.",
                    "danger",
                )

                return redirect(url_for("user.users"))

            # Delete Member first because Member.user_id
            # references User.id
