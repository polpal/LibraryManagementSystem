...
def validate_phone(form, field):
    ...
    if user and (not member or user.id != member.user_id):
        raise ValidationError("This phone number is already registered.")
...
class MemberForm(FlaskForm):
    ...
    member_no = StringField("Member No.", validators=[DataRequired(), Length(max=50)])

    name = StringField(
        "Name",
        validators=[DataRequired(), Length(min=2, max=150), validate_name_characters],
    )
    ...
    phone = StringField(
        "Phone", validators=[DataRequired(), Length(min=10, max=10), validate_phone]
    )
